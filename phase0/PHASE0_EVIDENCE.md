...
- At the time of the original snapshot, the historical Firebase file existed at `app/google-services.json`. The current working tree deliberately omits it so it cannot become active configuration. Only the sanitized, non-secret project/package facts retained in this register and `PROJECT_ABOUT.md` remain evidence; the credential-bearing historical bytes are not an evidence source and must be purged from every Git ref.
- `DESIGN.md` is now a tracked stable pointer to `PROJECT_ABOUT.md`; it contains no duplicate product authority.
...
### Sanitized historical configuration facts

- A prior audited snapshot recorded that the former tracked `app/google-services.json` was valid JSON and had SHA-256 `a890656e5723da63f187757fce398f645ff21d99d080488c56839748dda5473f`, matching the historical blueprint. The file is deliberately absent and its bytes must not be recovered or retained as evidence. The sanitized facts below and the matching records in `PROJECT_ABOUT.md` are the only tracked evidence source.
...
- Sanitized migration records show package divergence: the legacy Kotlin app used `com.aistudio.relateai.qxtjrk`, while a later React Native migration declared `com.relateai.app`. Those records do not establish a permanent application ID.

### External historical-credential remediation blocker

The complete-history secret gate currently detects a retired Google/Firebase API key in legacy paths that are reachable from remote refs. Deleting the files from the current tree did not remove that exposure. Before any release or production credential reuse:

1. the Firebase/Google Cloud owner must revoke or rotate the exposed key and verify application/API restrictions on every replacement;
2. a repository administrator must rewrite every affected branch and tag, force-update the coordinated remote refs, expire cached artifacts where supported, and require collaborators to replace stale clones rather than merge old history back;
3. the blocking full-history CI job must pass from a non-shallow checkout with no key-specific allowlist or exception; and
4. current tier-specific client configurations must be generated afresh in protected environments and must never be copied from the retired file.

This is an external credential-owner and repository-administrator action. Source changes can enforce the gate and retain sanitized facts, but cannot prove key rotation or complete remote/cache invalidation.
...
- the then-current 92-test Android instrumentation suite passed with no failures or skips on the API 29 ARM64 emulator, including a framework-owned launch of the real `MainActivity`; a separate Metro-backed install/launch then rendered the production setup UI with the Activity remaining resumed and no AndroidRuntime or React Native error. The source now declares 120 instrumentation tests, so this historical run is not a current-suite pass;
...
| Background/SMS     | Historical disposable target-36 and target-37 boundary runs passed their recorded 10/10 matrices, and an earlier candidate recorded 59-test API 36/API 37 emulator runs. A 92-test predecessor passed only on the API 29 ARM64 emulator; the current source declares 120 tests and has not been rerun as one suite on API 29, API 36, or API 37/16 KB. API 30–37 current-source coverage, installer allowlisting, and all physical SMS/carrier/soak evidence remain absent. | Current CI API 29/API 36/API 37 reruns plus the named channel, remaining API/OEM matrix, physical devices/SIMs, and nine-day soak |
