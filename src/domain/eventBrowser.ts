...
import { eventOccurrenceInYear, materializeEventOccurrence } from './occasionDates';
...
export const sortEventsByDate = (events: RelationshipEvent[], reference: Date = new Date()) =>
  events
    .map(event => materializeEventOccurrence(event, reference))
    .sort((a, b) => {
      const byDate = a.date.localeCompare(b.date);
      return byDate === 0 ? a.label.localeCompare(b.label) : byDate;
    });

export const filterRelationshipEvents = (events: RelationshipEvent[], filters: EventBrowserFilters) => {
  ...
  return sortEventsByDate(
    events
      .map(event => materializeEventOccurrence(event, nowDate))
      .filter(event => {
...
export const buildEventMonthView = (events: RelationshipEvent[], anchorIso: string): EventMonthView => {
