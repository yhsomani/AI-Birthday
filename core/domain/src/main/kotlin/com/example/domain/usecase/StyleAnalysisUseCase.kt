...
class StyleAnalysisUseCase @Inject constructor(
    ...
    suspend fun analyzeAndSave(texts: List<String>, source: String) {
        ...
        texts.forEach { text ->
            val words = text.lowercase().replace(NON_ALPHANUMERIC_REGEX, "").split(WHITESPACE_REGEX).filter { it.length > 2 }
    ...
    private fun findCommonPhrases(texts: List<String>): List<String> {
        ...
        texts.forEach { text ->
            text.lowercase().split(WHITESPACE_REGEX).distinct().forEach { word ->
                val cleanWord = word.replace(BASIC_NON_ALPHANUMERIC_REGEX, "")
    ...
    companion object {
        ...
        private val NON_ALPHANUMERIC_REGEX = Regex("[^a-zA-Z0-9\\s\u0900-\u097F]")
        private val BASIC_NON_ALPHANUMERIC_REGEX = Regex("[^a-zA-Z0-9]")
    }
}
