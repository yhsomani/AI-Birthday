...
class StyleAnalysisUseCase @Inject constructor(
    ...
    companion object {
        private val NON_ALPHANUMERIC_DEVANAGARI_REGEX = Regex("[^a-zA-Z0-9\\s\u0900-\u097F]")
    ...
    suspend fun analyzeAndSave(texts: List<String>, source: String) {
        ...
        texts.forEach { text ->
            val words = text.lowercase().replace(NON_ALPHANUMERIC_DEVANAGARI_REGEX, "").split(WHITESPACE_REGEX).filter { it.length > 2 }
