...
class StyleAnalysisUseCase @Inject constructor(
    ...
    suspend fun analyzeAndSave(texts: List<String>, source: String) {
        ...
        texts.forEach { text ->
            val words = text.lowercase().replace(NON_ALPHANUMERIC_OR_HINDI_PATTERN, "").split(WHITESPACE_PATTERN).filter { it.length > 2 }
        ...
        val greetings = texts.mapNotNull { text ->
            val words = text.trim().split(WHITESPACE_PATTERN)
    ...
    private fun findCommonPhrases(texts: List<String>): List<String> {
        ...
        texts.forEach { text ->
            text.lowercase().split(WHITESPACE_PATTERN).distinct().forEach { word ->
                val cleanWord = word.replace(NON_ALPHANUMERIC_PATTERN, "")
    ...
    companion object {
        ...
        private val NON_ALPHANUMERIC_OR_HINDI_PATTERN = Regex("[^a-zA-Z0-9\\s\u0900-\u097F]")
