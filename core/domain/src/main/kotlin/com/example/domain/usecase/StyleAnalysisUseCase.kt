...
class StyleAnalysisUseCase @Inject constructor(
    ...
    suspend fun analyzeAndSave(texts: List<String>, source: String) {
        ...
        texts.forEach { text ->
            val words = text.lowercase().replace(CLEAN_WORDS_PATTERN, "").split(WHITESPACE_PATTERN).filter { it.length > 2 }
    ...
    private fun findCommonPhrases(texts: List<String>): List<String> {
        ...
        texts.forEach { text ->
            text.lowercase().split(WHITESPACE_PATTERN).distinct().forEach { word ->
                val cleanWord = word.replace(ALPHANUMERIC_PATTERN, "")
    ...
    companion object {
        ...
        private val CLEAN_WORDS_PATTERN = Regex("[^a-zA-Z0-9\\s\u0900-\u097F]")
