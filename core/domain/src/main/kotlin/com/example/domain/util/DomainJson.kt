...
internal fun readJsonStringField(raw: String, key: String): String? {
...
internal fun encodeJsonStringArray(values: Iterable<String>): String {
    return values.joinToString(prefix = "[", postfix = "]") { value ->
        encodeJsonString(value)
    }
}

internal fun encodeJsonObject(fields: List<Pair<String, Any?>>): String {
    return fields.joinToString(prefix = "{", postfix = "}") { (key, value) ->
        "${encodeJsonString(key)}:${encodeJsonValue(value)}"
    }
}
