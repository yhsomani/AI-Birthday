...
## 2024-05-24 - Explicit Accessibility States for Disabled Inputs

**Learning:** React Native's `TextInput` does not automatically announce its disabled state to screen readers just by setting `editable={false}`.
**Action:** Always explicitly define `accessibilityState={{ disabled: inputProps.editable === false }}` and merge it with any incoming `accessibilityState` to ensure screen readers correctly interpret and announce the disabled state.
