...
## 2025-02-14 - Expose disabled state for AccessibleTextInput
**Learning:** In React Native, the `TextInput` component's disabled state is not inherently announced by screen readers solely through the `editable={false}` prop. We must manually map the computed `disabled` value to the component's `accessibilityState` prop (`disabled: editable === false`).
**Action:** When wrapping foundational React Native controls like `TextInput` to enhance accessibility, explicitly merge derived accessibility states (like `disabled`) with any incoming `accessibilityState` props to guarantee full screen reader coverage.
