...
## 2024-10-27 - Disabled State Accessibility

**Learning:** React Native's `TextInput` requires explicitly merging `inputProps.editable === false` into `accessibilityState.disabled` to ensure screen readers correctly announce the disabled state. Merely passing `editable={false}` is not enough.
**Action:** Always map `editable` to `accessibilityState.disabled` in base input components.
