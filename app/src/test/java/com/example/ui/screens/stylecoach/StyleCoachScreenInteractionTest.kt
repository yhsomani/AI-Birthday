...
class StyleCoachScreenInteractionTest {
    ...
    fun sampleTrainingAutoAnalysisBackProfileAndHistory_dispatchAndRenderExpectedState() {
        ...
        composeRule.onNodeWithText(context.getString(R.string.style_coach_confidence_growing))
            .assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(R.string.style_coach_samples_learned_value, 6))
            .assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(R.string.style_coach_language_hindi))
            .assertIsDisplayed()
        composeRule.assertLazyItemVisible(StyleCoachTestTags.STYLE_PREVIEW)
        composeRule.onNodeWithText(context.getString(R.string.style_coach_preview_title))
            .assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(R.string.style_coach_preview_opening_value, "Hey"))
            .assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(R.string.style_coach_preview_length_value, 42))
            .assertIsDisplayed()
        composeRule.onNodeWithText(context.getString(R.string.style_coach_preview_emoji_none))
            .assertIsDisplayed()
    ...
    fun blankBusyAndEmptyStates_renderExpectedControls() {
        ...
        state = StyleCoachUiState(profile = styleProfile().copy(sampleCount = 0))
        composeRule.assertLazyItemVisible(StyleCoachTestTags.PROFILE_CARD)
        composeRule.onNodeWithText(context.getString(R.string.style_coach_confidence_untrained))
            .assertIsDisplayed()
    }
