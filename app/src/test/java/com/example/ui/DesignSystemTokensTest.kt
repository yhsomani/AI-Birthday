...
class DesignSystemTokensTest {
    ...
    fun memoryVaultScreen_usesThemeBackedColorRoles() {
        val source = sourceDirectory("app/src/main/java/com/example/ui/screens/memoryvault")
            .walkTopDown().filter { it.isFile && it.extension == "kt" }.joinToString { it.readText() }
        ...
        assertTrue(
            "MemoryVaultScreen components should resolve note card colors through MaterialTheme.relateSemanticColors.",
            source.contains("MaterialTheme.relateSemanticColors"),
        )
        listOf(
            "RelateCard",
