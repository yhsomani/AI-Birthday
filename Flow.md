...
4. **Initial Recipient Selection (S07 -> S10)**: Multi-select contact picker (all contacts start *Off* by default) + template assignment.
...
2. Glance at **Status Hero Card**:
   - `Green`: *"Automation Active — Sending from this phone"*.
   - `Amber`: *"Attention Required — 1 issue needs repair"*.
   - `Neutral`: *"Automation Paused"*.
...
4. Selects tone (*Warm*, *Casual*, *Professional*, *Short*) -> Tap *Generate* -> View 3 candidate variations (zero PII sent).
