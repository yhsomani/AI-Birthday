...
| Full release-readiness validation | Pass | `JAVA_HOME=/opt/homebrew/opt/openjdk@21 ./gradlew testDebugUnitTest lintDebug assembleDebug jacocoDebugUnitTestReport --no-configuration-cache -Djavax.net.ssl.trustStore=.gradle/trust/cacerts-zscaler -Djavax.net.ssl.trustStorePassword=changeit`, rerun after the side-by-side debug package change; build successful with 241 unit tests, 0 failures, 0 errors, 0 skipped. |
...
| Home | settings, analytics, activity, style, backup, setup links; dashboard cards; sync error retry/dismiss | Not Run | Pending idle, unlocked device and seeded data. |
