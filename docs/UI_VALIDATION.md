...
|---|---|---|
| Unit tests and lint | Pass | Latest complete full validation produced 256 unit tests, 0 failures, 0 errors, 0 skipped after the Analytics CSV export interaction pass. |
...
| Analytics interaction and CSV export smoke | Pass | `JAVA_HOME=/opt/homebrew/opt/openjdk@21 ./gradlew :app:testDebugUnitTest --tests com.example.ui.screens.analytics.AnalyticsScreenInteractionTest --tests com.example.ui.screens.analytics.AnalyticsExportShareTest --tests com.example.ui.viewmodel.AnalyticsViewModelTest --no-configuration-cache -Djavax.net.ssl.trustStore=.gradle/trust/cacerts-zscaler -Djavax.net.ssl.trustStorePassword=changeit` passed tests covering activity navigation, export click, loading, empty, populated dashboard sections, disabled export state, and readable FileProvider-backed `text/csv` attachment sharing. |
...
| Full release-readiness validation | Pass | `JAVA_HOME=/opt/homebrew/opt/openjdk@21 ./gradlew testDebugUnitTest lintDebug assembleDebug jacocoDebugUnitTestReport --no-configuration-cache -Djavax.net.ssl.trustStore=.gradle/trust/cacerts-zscaler -Djavax.net.ssl.trustStorePassword=changeit`, rerun after the Analytics CSV export interaction coverage change; build successful with 256 unit tests, 0 failures, 0 errors, 0 skipped. |
| Debug Android-test APK build | Pass | `JAVA_HOME=/opt/homebrew/opt/openjdk@21 ./gradlew :app:assembleDebugAndroidTest --no-configuration-cache -Djavax.net.ssl.trustStore=.gradle/trust/cacerts-zscaler -Djavax.net.ssl.trustStorePassword=changeit` passed after the Analytics CSV export UI refactor. |
...
| Analytics | activity link, export/share action, empty and populated states | Pass | `AnalyticsScreenInteractionTest` covers the activity-history button, export button, loading indicator, empty monthly/neglected states, populated stat sections, growth metrics, and neglected contacts. `AnalyticsExportShareTest` verifies export creates a readable cache-backed FileProvider CSV attachment. Live device share-sheet selection still requires an idle, unlocked device. |
