...
| Write-contacts, legacy external-storage, and overlay permissions | Not required by the RN replacement. Contact import is read-only; file import/export uses user-controlled document and share surfaces.                                                  | None through these permissions.                                                                | `app.json` blocks `WRITE_CONTACTS`, `READ_EXTERNAL_STORAGE`, `WRITE_EXTERNAL_STORAGE`, and `SYSTEM_ALERT_WINDOW` from merged release manifests.                                                                                                                             |
...
The directly invoked `src/config/releaseEvidenceCli.ts` must show an empty top-level generated-native/legacy artifact path list for the active RN release. Any checked-in `android/` or `ios/` tree or reintroduced legacy path is release evidence drift and must be removed or explicitly resolved before signing.
...
- Android platform auto backup is disabled for the entire application because relationship state and protected key material must never be copied independently of the explicit encrypted-backup workflow.
