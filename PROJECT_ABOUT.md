...
- [Apple background task scheduling](https://developer.apple.com/documentation/uikit/using-background-tasks-to-update-your-app)
- [Google Sign-In for iOS API access and token refresh](https://developers.google.com/identity/sign-in/ios/api-access)
...
**Play-signing rule:** Google Play AABs are signed with an upload key, but Play-delivered APKs are signed with the distinct Play app-signing key. Authority evidence, Gradle, runtime readiness, Firebase/OAuth/App Check registration, and physical-device verification must never conflate them. A Play build validates the packaged AAB against the upload certificate while embedding and later verifying the approved installed Play app-signing certificate; the post-build gate must also decode the AAB base manifest with the locked official bundletool and independently prove the exact package, version, SDK, release flags, restricted permissions, and forbidden-permission absence. Operator-supplied coordinates or ZIP entry names are not artifact evidence. A direct/managed APK has no upload-certificate field and its artifact signer equals its installed signer. Internal App Sharing or another Play test surface that re-signs with a test certificate is never production distribution evidence.
...
1. Show a prominent disclosure immediately before authorization: the raw contact dataset stays in encrypted device storage. On Android only, an enabled due recipient contributes the fixed-length pseudonymous safety bases described in Section 8.5 to Firebase coordination; the raw name, People ID, birthday, phone, and message never cross that boundary. The iOS Companion does not derive or transmit those coordination bases.
...
9. Schedule platform-appropriate background refresh after a successful foreground authorization/sync. Android uses connected-network periodic WorkManager and iOS uses a registered `BGAppRefreshTask`, whose launch time remains entirely system-controlled and best effort. Neither path may present consent or an account chooser in the background. If Google requires foreground resolution, stop retrying, cancel that platform's pending refresh request, and add an attention item; a later successful foreground reconnect/sync schedules the next request again.
...
**Failures:** Android handles one 401 by clearing that exact rejected access token through Google's Android authorization client, acquiring a distinct SDK token, and retrying once; a repeated 401 stops. On iOS, every request first uses Google Sign-In's supported `refreshTokensIfNeeded` path. Google Sign-In for iOS exposes no supported forced refresh or single-access-token eviction for an otherwise unexpired bearer rejected by People, so iOS never replays it: it signs out the Google SDK session, invalidates the exact protected account session, and queues one foreground reconnect while leaving no background authorization loop. On either platform, if interactive resolution is required, stop and queue foreground reconnect. A 403, 429, expired sync token, pagination interruption, parameter mismatch, malformed person, empty account, partial source merge, offline state, or cancellation never enters an unbounded loop.
...
5. After `MFMessageComposeViewController.canSendText()` passes, native iOS revalidates the exact protected proposal and People snapshot, takes a short local lease on that material, refreshes the content-free account-global coexistence status, and acquires a `PREPARED` iOS composer reservation with its protected exact-owner capability. That server transaction serializes against account deletion and every Android sender mutation; an Android binding, unresolved coordination state, deletion state, unavailable continuity, or another live capability refuses the composer.
6. Immediately before presentation, Swift rechecks the account, proposal, material generation, lease, foreground scene, nonce, and coexistence result; durably makes the local capability sticky before asking the server to commit the reservation; receives an exact `COMMITTED` response; compare-and-sets the protected proposal to `ComposerOpenCommitted`; revalidates and retains the People lease; and only then presents `MFMessageComposeViewController`. The system composer remains editable; the user controls recipient/body edits and Send or Cancel, while iOS/Messages controls which sender line and SMS/MMS/iMessage options, if any, are available.
7. Activity records only `Composer opened`, `Composer cancelled`, `Composer failed`, `Composer outcome unknown`, or `Composer reported sent`. A reported-sent result means MessageUI reported the user-edited message queued or sent; it does not expose the final edited payload or prove carrier acceptance or delivery. Process death, backgrounding, forced safety dismissal, or a lost delegate result becomes Unknown, never inferred success or failure and never an automatic retry.

**Native composer state machine:** React Native never passes a free-form recipient/body directly to a presentation-only native API. After a current foreground review, it passes only the protected proposal ID, expected revision, and one-use foreground action nonce. Swift rechecks the proposal, exact account, account-global coexistence state, reset safety, terminal marker, current People snapshot generation, foreground scene, and nonce before leasing the exact contact material and acquiring the server reservation. It then performs the sticky reservation sequence and compare-and-sets `ProposalReady -> ComposerOpenCommitted` with one random operation ID before presentation. Exactly one operation may be in flight. Presentation completion advances to `ComposerPresented`. A definitive presentation failure advances to `ComposerFailed`; a delegate result transaction advances to `ComposerCancelled`, `ComposerReportedSent`, `ComposerFailed`, or `ComposerOutcomeUnknown` before the app dismisses/resolves. Store failure, process death, stale session generation, lost delegate, app backgrounding, the reservation safety deadline, or an `OpenCommitted/Presented` operation found on a later clean launch becomes `ComposerOutcomeUnknown`. Duplicate taps, stale revisions/nonces, wrong-scene requests, privacy wipe or suspended Contacts access, and duplicate/out-of-order delegate results are idempotently refused.
...
**Account-global composer reservation:** companion status is advisory preflight; the reservation is the authoritative cross-platform race fence. The server stores one top-level `iosComposerReservations/{uid}` document containing only schema version, a domain-separated SHA-256 owner capability key, `PREPARED` or `COMMITTED` phase, ledger generation, and logical/cleanup timestamps. It stores no contact, date, destination, phone, recipient, body, message, prompt, or raw owner capability. The protected, backup-excluded iOS journal binds one random lowercase UUID capability to salted digests of the exact Firebase UID and Google subject plus the local account generation; raw account identifiers and message material never enter that journal or React Native.

Each successful exact-owner acquisition sets a server-authoritative logical deadline to server time plus 72 hours. Firestore TTL is delayed physical cleanup only and never authorizes Android or iOS. The same exact owner capability may revalidate and reuse its reservation; another capability, including the same phone after its local journal was wiped or lost, is refused while the old reservation remains live. Only an exact `PREPARED` reservation that has never begun the sticky transition may be released early, and the client clears its capability only after an exact authenticated `RELEASED` response. Before the commit callable is attempted, iOS durably marks the local capability sticky. From that point, timeout, cancellation, crash, presentation failure, reported Sent, Unknown, sign-out, revoke, or local wipe cannot authorize early release. A live `COMMITTED` reservation remains sticky until logical expiry; account deletion is the only operation that transactionally dominates and removes it early.

Every Android registration, lease, mode change, claim, Arm/status seal, report, safe retry, transfer, and new or in-progress destructive reset/release transaction reads the same reservation document and refuses while it is live. Exact unexpired `COMPLETED` reset/release receipts remain replayable because they are immutable privacy proof rather than a new Android mutation. The first concurrent iOS acquisition or Android registration/mutation wins the shared Firestore transaction; the loser fails closed. MessageUI is dismissed to Unknown when the app resigns active and, using monotonic elapsed time with full request round-trip subtraction, at least five minutes before the server expiry. This prevents an editable composer from remaining open when Android could become eligible again.
...
**Privacy and deletion:** disconnect, revoke, sign-out, local wipe, and account deletion apply the same truthful data-boundary copy. iOS additionally cancels app-owned notification requests, removes protected local stores, the composer owner-capability journal, and Keychain material in the documented crash-recoverable order, verifies their absence, and never claims to delete Messages, carrier, recipient, iCloud, or other external copies. Destroying the local reservation journal does not contact the server or release a live fence; Android may remain safely paused for up to the remaining 72-hour logical window, and the UI describes the holder neutrally as another iPhone or an earlier protected review because those cases are intentionally indistinguishable. Account deletion transactionally removes the top-level server reservation when deletion starts and verifies its absence during orchestrated account cleanup.

**Acceptance:** physical-iPhone tests prove no background or programmatic SMS path; no free-form JS-to-composer presentation path; no composer without a current foreground review, matching proposal revision, one-use nonce, explicit tap, exact People material lease, successful `PREPARED -> COMMITTED` server reservation, and native CAS; crash injection before/after local capability creation, acquire response, local sticky transition, server commit, OpenCommitted, presentation, delegate persistence, dismissal, background/foreground, and process restart; double-tap, duplicate/out-of-order delegate, wrong-scene, `canSendText`, cancelled, failed, reported-sent, and lost-result Unknown behavior; monotonic five-minute-early expiry dismissal; exact-owner reuse, another/lost capability refusal, PREPARED-only release, and zero early release after sticky transition; terminal-marker survival after Clear activity; reset-date timezone travel, clock rollback, eight-entry release, ninth-entry fail-closed, wipe/restore/corrupt-ledger suppression; wipe recovery proving the reservation journal is absent without claiming server release; no claim about the final sender line, SMS/MMS/iMessage transport, payload, carrier acceptance, or delivery; deterministic 400-day/60-request horizon ordering, opaque notification payloads, add/query/remove partial failure, denial/delay/Focus/concurrent reconciliation; VoiceOver and Dynamic Type; secure backup/deletion behavior; account-global suppression for an unrelated unresolved Android occurrence; real concurrent iOS-acquire-versus-first-Android-registration and every-mutation races; exact completed privacy-receipt replay while a later reservation is live; and account deletion racing and removing the reservation.
...
- The reviewed Gemini implementation candidate is Vertex AI through Firebase AI Logic with stable `gemini-3.5-flash` in location `global` (required by this Gemini 3.x model), Android `firebase-ai` 17.13.0 with Play Integrity App Check 19.2.0 and Remote Config 23.1.0, and Apple `FirebaseAILogic`/`FirebaseRemoteConfig` 12.15.0 in the same exact Firebase family as Auth/App Check/Functions. Both clients default the native-only `gemini_suggestions_enabled` switch Off and enable only an activated canonical remote boolean `true`; built-in templates remain available. These pins do not satisfy the release gate by themselves; signed-channel attestation, Remote Config template approval, model availability, billing/quota, authenticated-users mode, replay enforcement, and monitoring-Off evidence remain mandatory.
- Dependency versions are exact in owned manifests and pinned through npm, Bundler, CocoaPods, and strict application Gradle locks; Gradle artifact bytes are independently SHA-256 verified for the complete build/test/lint graph and both macOS/Linux host tools. Direct selection or use of preview models, SDKs, or APIs is forbidden in production. A beta-labeled vendor-internal transitive module that the required stable SDK itself registers is not a product-selected preview API: it must remain unreferenced by app code, appear in the SBOM and release dependency review, and be rechecked against the upstream stable SDK before every release. Release remains blocked if that transitive surface becomes callable or exposed by the app, is no longer supported as part of the stable SDK, or cannot pass the normal security and compatibility review.
...
**Binding scope:** callable Cloud Functions is the only client-facing coordination API. A server-only Firestore ledger uses transactions to fence an active Android installation epoch and reserve each logical recipient occurrence plus a second destination/date guard. It also enforces the immutable MVP ceiling of 20 newly armed occurrences per UID in any rolling 24 hours, independent of the softer user-configured local cap. Functions requires Firebase Auth and enforced App Check with consumable limited-use tokens. The native SDK attaches those credentials; none enters React Native JavaScript or user workflows. Mobile Firestore rules deny all reads and writes. iOS uses a read-only, content-free companion-status callable as advisory preflight and the three authenticated `acquireIOSComposerReservation`, `commitIOSComposerReservation`, and `releaseIOSComposerReservation` callables as the authoritative account-global race fence. Their requests carry only contract version, reviewed ledger generation where applicable, and a random owner capability UUID—never a contact, date, destination, or message basis. Any live Android binding, unresolved permit, deletion/operation state, incompatible continuity, or another live iOS capability suppresses every in-app iOS composer action, including for unrelated recipients. Unknown or unavailable state also suppresses every in-app composer rather than weakening Android fencing; iOS never receives an Android epoch, claim, Arm, or permit.
...
| Entity              | Key fields                                                                                                                                                                                                                                                                                                                                                           |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Account             | local ID, Google subject hash, Firebase UID, display email, attachment-time locale metadata (non-authoritative; current device locale drives presentation and phone-region normalization), active state, durable monotonic blockerRevision                                                                                                                           |
...
- Screenshot and recents protection uses an opaque, content-free app-switcher cover on iOS and disables Android recents screenshots (with a paused-only `FLAG_SECURE` fallback on older Android releases). Active foreground screenshots remain user-controlled for accessibility and support: iOS provides no supported general screenshot-prevention API, and Android does not globally apply `FLAG_SECURE` while active. The privacy UI discloses that an active screenshot or screen recording can contain visible contact/message detail, so the user must review it before sharing.
...
- The read-only companion-status service provides advisory iOS preflight, while the content-free account-global iOS composer reservation serializes MessageUI presentation against every Android sender mutation; neither service turns MessageUI into a remote send service or gives iOS an Android epoch, claim, Arm, or permit.
...
- iOS Google/Firebase identity and read-only Contacts consent; protected-store migration/key loss; deterministic 400-day/60-request notification horizon, opaque request payloads, allow/deny/delay/Focus/partial/concurrent reconciliation; native foreground proposal CAS and exact People-material lease; `canSendText`; editable recipient/body plus unknowable sender line/transport; cancel/failed/reported-Sent/lost-delegate Unknown; crash boundaries, terminal-marker retention, fresh-install/wipe/restore/corrupt-ledger reset-date timezone/clock/overflow suppression; account-global Android-managed and unavailable coexistence status; exact-owner `PREPARED -> COMMITTED` composer reservation, 72-hour logical expiry, five-minute-early/background dismissal, capability loss and sticky no-release behavior; every Android mutation race; background/terminated launch; local wipe/Keychain/notification/reservation-journal cancellation and verified absence; iOS-only deletion racing first Android registration and live reservation; and proof that no free-form JavaScript or background/programmatic SMS API exists;
...
- Zero iOS background, free-form JavaScript, or programmatic SMS path and zero MessageUI presentation without an explicit current foreground review action, matching proposal revision/one-use nonce, exact People-material lease, a successful account-global `PREPARED -> COMMITTED` reservation, and durable native OpenCommitted CAS. Composer cancellation, failure, lost result, and reported-Sent are distinct; none proves final sender line, SMS/MMS/iMessage transport, carrier acceptance, delivery, or the final edited payload. Reported-Sent/Unknown cannot reopen the same in-app occurrence, Clear activity preserves the minimum terminal marker, and missing/reset history cannot open a blocked-date composer.
- Every iOS coexistence preflight and reservation acquisition suppresses the in-app composer while any live Android sender binding, in-flight/unresolved occurrence, deletion/coordination operation, incompatible continuity, another live owner capability, or unavailable status exists; iOS never receives a sender epoch, Android claim, Arm, or permit.
- iOS coexistence suppression is account-global because neither status nor reservation carries a recipient/date/destination/message basis: an unrelated unresolved Android occurrence suppresses every in-app iOS composer action, every Android sender mutation transaction refuses a live iOS reservation, and no UI may imply per-recipient matching.
- A never-committed exact `PREPARED` reservation may release only after an exact authenticated response. Local sticky state precedes the commit attempt; every committed or ambiguous-sticky reservation survives cancel/fail/reported-Sent/Unknown/crash/local wipe until server logical expiry, while account deletion may transactionally remove it. Firestore TTL delay is never authority. MessageUI is dismissed to Unknown on background and at least five minutes before expiry using a monotonic deadline.
...
- Every signed Android build from an actual channel passes Firebase AI Logic and every state-changing callable Function through enforced Play Integrity App Check and limited-use-token replay protection. Every signed iOS build passes AI Logic, companion status, and all three composer-reservation callables through the approved production iOS App Check provider with equivalent replay enforcement where supported; no debug provider ships.
...
7. Enable People API and configure OAuth brand, domains, privacy, terms, support, and contacts.readonly. Test partial grant; Android's one bounded clear-token/reauthorize 401 recovery; iOS's no-replay foreground-reconnect response to a rejected current bearer; local disconnect; and the disclosed all-application-scope effect of revokeAccess.
...
11. Enforce App Check baseline and limited-use-token replay protection for Firebase AI Logic and every state-changing callable Function, plus the iOS read-only companion-status callable. This includes acquire, commit, and release of the iOS composer reservation. A signed build from every real Android and iOS channel must successfully attest, call Gemini, and complete its applicable non-SMS coordination/status/reservation probe before that channel is supported.
...
- **Coordination gate:** accept that Android unattended submission needs online claim/arm, then prove TEST_ONLY/PAUSED_REPAIR versus AUTOMATION_ACTIVE lease-renewal predicates, active sender epoch, recipient claim, destination guard, old/new canonicalization aliases, claim-known maximum/actual/effective permit deadlines and monotonic boot anchors, local barrier, two-device TRANSFER_PENDING and authenticated/admin DELETING drains, outage/Missed behavior, hard server arm cap, exact control-record lifecycles, iOS read-only Android-managed preflight plus the authoritative account-global `PREPARED -> COMMITTED` composer reservation against every Android mutation, and zero raw contact/message fields or logs.
...
- **iOS companion gate:** name the bundle ID, deployment target, supported iPhone matrix, native Google/People implementation, protected store, notification horizon/reconciliation, MessageUI bridge, App Check provider, App Store privacy/entitlement declarations, current login-services review rationale/acceptance, and Android-managed coexistence behavior. Prove no background/programmatic SMS path; exact People-material leasing; account-global `PREPARED -> COMMITTED` reservation ordering, 72-hour logical expiry, five-minute-early/background dismissal, PREPARED-only early release, sticky crash/wipe behavior, every Android mutation and deletion race; truthful cancel/fail/Sent/Unknown outcomes; notification/Keychain/reservation-journal cancellation and verified absence after sign-out/revoke/wipe/account deletion without claiming server release; and no silent restore on physical iPhones.
