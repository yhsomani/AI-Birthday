...
export const authorizeSafeRetry = onCall(
  ...
  async (request: CallableRequest<z.infer<typeof retrySchema>>) => {
    ...
    const outcome = await db.collection('users').doc(uid)
      .collection('smsOutcomes').doc(occurrenceUuid).get();
...
```json
{
  "diagnostics": {
    ...
    "appVersion": "0.1.0",
    "recentActivity": [ /* last 50 entries */ ],
    "errors": [ /* last 20 error logs */ ],
    "phoneNumbers": [ /* sender numbers */ ],
    "birthdays": [ /* count only, no names/dates */ ]
  }
}
```
...
const colors = {
  foreground: useColorScheme() === 'dark' 
...
const iosComposerReservation = (uid: string) =>
  db.collection('users').doc(uid).collection('iosState')
