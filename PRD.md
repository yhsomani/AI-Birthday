# RelateAI System Documentation & Rebuild Blueprint
...
- Setup multi-module architecture (Clean Architecture)
- Setup Hilt DI, Room DB with SQLCipher, and EncryptedSharedPreferences
- Define domain models and repository interfaces

**Phase 2: Core Integrations (Weeks 3-4)**
- Implement Google People API integration for contact sync
- Implement Gemini API integration for message generation with prompt building
- Set up WorkManager for background sync and scheduling

**Phase 3: Database & Logic (Weeks 5-6)**
- Implement Contact Merging logic
- Implement classification logic (relationship, tone)
- Build background job workers (Discovery, Generation, Dispatch)

**Phase 4: Senders & Accessibility (Week 7)**
- Implement SMS Sender
- Implement Email (SMTP) Sender
- Implement WhatsApp Accessibility Service sender

**Phase 5: UI & Features (Weeks 8-10)**
- Build Dashboard, Contact Detail, and Events screens
- Implement approval flows (Smart Approve, Always Ask)
- Build onboarding and setup screens

**Phase 6: Hardening & Release (Weeks 11-12)**
- Write unit tests for business logic and parsers
- Polish UI/UX, animations, empty states
- Pre-launch testing and Play Store compliance fixes

### Testing Strategy
- Unit test business logic, use cases, view models.
- Mock external APIs (Google, Gemini) for robust tests.
- UI tests for critical flows (onboarding, approval).

### Deployment Strategy
- Use GitHub Actions for CI/CD.
- Manage Keystore separately, sign release build automatically.
- Internal testing track -> Closed Beta -> Staged Rollout -> 100% Prod.


## 10. Developer Handoff Documentation

### Complete Functional Specifications
Refer to the Feature Analysis section. Rebuild must achieve feature parity.

### Acceptance Criteria
- Must pass lint, detekt, and have >60% test coverage.
- No DB access or MasterKey derivation on the main thread (CRIT-01).
- Encrypted Backup (CRIT-03).
- Offline capabilities (Local-First), except Gemini inference and cloud syncing.
- Background workers must respect doze and battery optimisations.

### Assumptions and Dependencies
- Relies heavily on Google Contacts data quality.
- WhatsApp structural changes might break the Accessibility Service sender.
- Requires valid Gemini API Key from the user.
